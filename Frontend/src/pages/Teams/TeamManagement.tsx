...
import { AnnounceModal } from "@/components/Teams/AnnounceModal";
...
export function TeamManagement({ userRole }: TeamManagementProps) {
  ...
  const [teamForAnnounce, setTeamForAnnounce] = useState<TeamListDto | null>(null);
  ...
  const loadTeams = useCallback(async () => {
    try {
      ...
      toast.error("팀 목록을 불러오는 데 실패했습니다. 잠시 후 다시 시도해주세요.");
      setTeams([])
    } finally {
      setLoading(false);
    }
  ...
  const handleInviteSent = (invitedUser: UserDto) => {
  ...
  const OnAnnounce = (team: TeamListDto) => {
    setTeamForAnnounce(team);
    setShowAnnounceModal(true);
  };

  const handleSendAnnouncement = async (title: string, content: string) => {
    if (!teamForAnnounce) return;
    await sendTeamAnnouncement(teamForAnnounce.id, title, content);
  };
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="flex gap-2">
                  ...
                  {userRole === "professor" && (
                    <>
                      ...
                      <Button size="sm" variant="outline" onClick={() => OnAnnounce(t)}>
                        <MessageSquare className="h-4 w-4 mr-1" /> 공지
                      </Button>
                    </>
                  )}
                </div>
      ...
      <TeamSettingsModal
        ...
        onClose={() => { setShowSettingsModal(false); setSelectedTeam(null); }}
        team={selectedTeam}
        handlers={{
