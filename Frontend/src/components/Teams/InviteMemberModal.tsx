...
import { inviteMemberRequest } from "@/api/teams";
...
interface InviteMemberModalProps {
    ...
    users: UserDto[];
    isLoading: boolean;
    onInviteSent: (invitedUser: UserDto) => void;
    // onInviteSuccess: (teamId: number, newUser: UserDto) => void;
}

export function InviteMemberModal({ isOpen, onClose, team, users, isLoading, onInviteSent }: InviteMemberModalProps) {
    ...
    const handleInvite = async (userToInvite: UserDto) => {
        ...
        try {
            await inviteMemberRequest(team.id, userToInvite.id);

            onInviteSent(userToInvite);
            // onInviteSuccess(team.id, userToInvite);
            // toast.success(`${userToInvite.name} 님을 팀에 초대했습니다.`);
            toast.success(`${userToInvite.name} 님께 초대 요청을 보냈습니다.`);
        } catch (error) {
            console.error("[초대 요청 오류]Team invitation error:", error);
            toast.error("초대 요청을 보내는 중 오류가 발생했습니다.");
                            ...
                            <div className="space-y-2 pr-4">
                                {filteredUsers.length > 0 ? (
                                    filteredUsers.map((user) => {
                                        ...
                                        const isAlreadyInvited = user.invitationStatus === 'PENDING';
                                        return (
                                            <div key={user.id} className="flex items-center justify-between p-2 border rounded-md">
                                                ...
                                                <Button size="sm"
                                                    ...
                                                    disabled={isInviting || invitingUserId !== null || isAlreadyInvited}
                                                >{isInviting ? (
                                                    <>
