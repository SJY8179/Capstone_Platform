...
export async function fetchNotifications(opts: FetchOpts = {}): Promise<AppNotification[]> {
    ...
    const [projectBasedNotifications, teamBasedNotifications] = await Promise.all([

        // --- Promise 1: 기존 프로젝트 기반 알림 집계 로직 ---
        (async () => {
...
/** 초대 응답 */
export async function respondToInvitation(invitationId: number, accept: boolean): Promise<void> {
    await http.post('/notifications/invitations/respond', { invitationId, accept });
}