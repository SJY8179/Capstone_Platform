...
import { updateTeam, changeLeader, removeMember, deleteTeam, getAllProfessors, addProfessorToTeam } from '@/api/teams';
...
export function TeamSettingsModal({
  ...
  useEffect(() => {
    if (team) {
      ...
      if (leader) setSelectedLeaderId(String(leader.id));
  ...
  const loadProfessors = async () => {
    try {
      ...
      const teamProfessorIds =
        team?.members.filter(m => professorList.some(p => p.id === m.id)).map(m => m.id) || [];
      const availableProfessors = professorList.filter(p => !teamProfessorIds.includes(p.id));
      setProfessors(availableProfessors);
    } catch (error) {
      const { message } = getApiError(error);
      toast.error(`교수 목록 불러오기 실패: ${message}`);
    }
  };
  ...
  const handleUpdateTeamInfo = async () => {
    ...
    try {
      ...
      const { message } = getApiError(error);
      toast.error(`팀 정보 수정 실패: ${message}`);
    } finally {
  ...
  const handleLeaderChange = async (newLeaderId: string) => {
    ...
    if (!newLeader) return;
    if (!window.confirm(`정말 ${newLeader.name}님을 새로운 팀장으로 임명하시겠습니까?`)) return;
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-xl h-[70vh] flex flex-col p-0">
        <DialogHeader className="p-6 pb-4 border-b">
          ...
          <DialogDescription>원하는 설정 탭을 선택하세요.</DialogDescription>
        </DialogHeader>
            ...
            {activeView === 'members' && (
              <Card className="border-none shadow-none">
                <CardContent className="pt-6 space-y-6">
                  <div className="space-y-2">
                    <Label htmlFor="leader-select" className="flex items-center gap-1">
                      <Crown className="w-4 h-4 text-yellow-500" /> 팀 리더
                    </Label>
                    <Select value={selectedLeaderId} onValueChange={handleLeaderChange} disabled={isSubmitting}>
                      ...
                      <SelectContent>
                        {team.members.filter(m => m.userRole !== 'TA').map(m =>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <Label>팀원 목록</Label>
                      <Button size="sm" variant="outline" onClick={() => setIsAddingProfessor(!isAddingProfessor)} disabled={isSubmitting}>
                    ...
                    {isAddingProfessor && (
                      <div className="flex gap-2 p-3 border rounded-md bg-muted/20">
                        <Select value={selectedProfessorId} onValueChange={setSelectedProfessorId} disabled={isSubmitting}>
                          <SelectTrigger className="flex-1">
                            <SelectValue placeholder="교수를 선택하세요" />
                          </SelectTrigger>
                          <SelectContent>
                            {professors.length === 0 ? (
                              <div className="p-2 text-sm text-muted-foreground">추가 가능한 교수가 없습니다</div>
                            ) : (
                              professors.map(prof => (
                        ...
                        <Button size="sm" onClick={handleAddProfessor} disabled={!selectedProfessorId || isSubmitting}>추가</Button>
                        <Button size="sm" variant="outline" onClick={() => { setIsAddingProfessor(false); setSelectedProfessorId(''); }} disabled={isSubmitting}>취소</Button>
                      </div>
                    )}
                    ...
                    <ScrollArea className="h-[200px] rounded-md border">
                      <div className="space-y-2 max-h-[200px] overflow-y-auto border rounded-md p-2">
                        ...
                        {team.members.filter(m => m.role !== 'leader' && m.userRole !== 'TA').length === 0 &&
                          <p className="text-sm text-muted-foreground p-2">팀원이 없습니다.</p>}
                      </div>
                    </ScrollArea>
                  </div>
                </CardContent>
